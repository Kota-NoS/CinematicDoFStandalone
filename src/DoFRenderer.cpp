...
namespace
{
	...
	void LogSilhouetteOutputDiagnostics(
		RE::BSGraphics::RendererWindow* a_window,
		ID3D11Texture2D* a_destinationTexture)
	{
		if (!a_window || !a_destinationTexture) {
			return;
		}

		D3D11_TEXTURE2D_DESC textureDescription{};
		a_destinationTexture->GetDesc(&textureDescription);

		D3D11_RENDER_TARGET_VIEW_DESC viewDescription{};
		if (a_window->renderView) {
			auto view = reinterpret_cast<ID3D11RenderTargetView*>(a_window->renderView);
			view->GetDesc(&viewDescription);
		}

		auto swapChain = reinterpret_cast<IDXGISwapChain*>(a_window->swapChain);
		if (!swapChain) {
			spdlog::warn(
				"Silhouette DXGI diagnostics: no swap chain; textureFormat={}, rtvFormat={}",
		...
		const auto descriptionResult = swapChain->GetDesc(&description);
		spdlog::info(
			"Silhouette DXGI diagnostics: texture={}x{}, textureFormat={}, rtvFormat={}, descHr=0x{:08X}, "
			"buffer={}x{}, bufferFormat={}, windowed={}, swapEffect={}, bufferCount={}, flags=0x{:X}, samples={}x{}",
			...
			static_cast<std::uint32_t>(description.BufferDesc.Format),
			description.Windowed != FALSE,
			static_cast<std::uint32_t>(description.SwapEffect),
		...
		const auto swapChain1Result = swapChain->QueryInterface(IID_PPV_ARGS(swapChain1.GetAddressOf()));
		if (SUCCEEDED(swapChain1Result)) {
			DXGI_SWAP_CHAIN_DESC1 description1{};
			const auto description1Result = swapChain1->GetDesc1(&description1);
			spdlog::info(
				"Silhouette DXGI 1.1: queryHr=0x{:08X}, descHr=0x{:08X}, format={}, stereo={}, "
				"scaling={}, swapEffect={}, alphaMode={}, bufferCount={}, flags=0x{:X}",
				static_cast<std::uint32_t>(swapChain1Result),
				static_cast<std::uint32_t>(description1Result),
				static_cast<std::uint32_t>(description1.Format),
				description1.Stereo != FALSE,
				static_cast<std::uint32_t>(description1.Scaling),
				static_cast<std::uint32_t>(description1.SwapEffect),
				static_cast<std::uint32_t>(description1.AlphaMode),
				description1.BufferCount,
				description1.Flags);
		} else {
			spdlog::info(
				"Silhouette DXGI 1.1 interface unavailable: queryHr=0x{:08X}",
				static_cast<std::uint32_t>(swapChain1Result));
		}

		ComPtr<IDXGISwapChain3> swapChain3;
		const auto swapChain3Result = swapChain->QueryInterface(IID_PPV_ARGS(swapChain3.GetAddressOf()));
		if (SUCCEEDED(swapChain3Result)) {
			UINT g22Support = 0;
			UINT linearSupport = 0;
			UINT hdr10Support = 0;
			const auto g22Result = swapChain3->CheckColorSpaceSupport(
				DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709,
				&g22Support);
			const auto linearResult = swapChain3->CheckColorSpaceSupport(
				DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709,
				&linearSupport);
			const auto hdr10Result = swapChain3->CheckColorSpaceSupport(
				DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020,
				&hdr10Support);
			spdlog::info(
				"Silhouette DXGI color-space support: queryHr=0x{:08X}, "
				"G22(hr=0x{:08X}, flags=0x{:X}), linear(hr=0x{:08X}, flags=0x{:X}), "
				"HDR10(hr=0x{:08X}, flags=0x{:X})",
				static_cast<std::uint32_t>(swapChain3Result),
				static_cast<std::uint32_t>(g22Result),
				g22Support,
				static_cast<std::uint32_t>(linearResult),
				linearSupport,
				static_cast<std::uint32_t>(hdr10Result),
				hdr10Support);
		} else {
			spdlog::info(
				"Silhouette DXGI 1.4 interface unavailable: queryHr=0x{:08X}",
				static_cast<std::uint32_t>(swapChain3Result));
		}

		BOOL fullscreen = FALSE;
		ComPtr<IDXGIOutput> output;
		const auto fullscreenResult = swapChain->GetFullscreenState(&fullscreen, output.GetAddressOf());
		if (!output) {
			const auto containingOutputResult = swapChain->GetContainingOutput(output.GetAddressOf());
			spdlog::info(
				"Silhouette DXGI output lookup: fullscreenHr=0x{:08X}, fullscreen={}, containingOutputHr=0x{:08X}",
				static_cast<std::uint32_t>(fullscreenResult),
				fullscreen != FALSE,
				static_cast<std::uint32_t>(containingOutputResult));
		} else {
			spdlog::info(
				"Silhouette DXGI output lookup: fullscreenHr=0x{:08X}, fullscreen={}, output supplied by GetFullscreenState",
				static_cast<std::uint32_t>(fullscreenResult),
				fullscreen != FALSE);
		}
		...
		const auto outputDescriptionResult = output->GetDesc(&outputDescription);
		spdlog::info(
			"Silhouette DXGI output: descHr=0x{:08X}, desktop=({}, {})-({}, {}), attached={}, rotation={}",
			static_cast<std::uint32_t>(outputDescriptionResult),
			...
			outputDescription.AttachedToDesktop != FALSE,
			static_cast<std::uint32_t>(outputDescription.Rotation));

		DXGI_GAMMA_CONTROL_CAPABILITIES gammaCapabilities{};
		const auto gammaCapabilitiesResult = output->GetGammaControlCapabilities(&gammaCapabilities);
		spdlog::info(
			"Silhouette DXGI gamma capabilities: hr=0x{:08X}, scaleOffset={}, min={}, max={}, points={}",
			static_cast<std::uint32_t>(gammaCapabilitiesResult),
			gammaCapabilities.ScaleAndOffsetSupported != FALSE,
			gammaCapabilities.MinConvertedValue,
			gammaCapabilities.MaxConvertedValue,
			gammaCapabilities.NumGammaControlPoints);

		DXGI_GAMMA_CONTROL gammaControl{};
		const auto gammaControlResult = output->GetGammaControl(&gammaControl);
		if (SUCCEEDED(gammaControlResult)) {
			const auto& quarter = gammaControl.GammaCurve[256];
			const auto& middle = gammaControl.GammaCurve[512];
			const auto& threeQuarter = gammaControl.GammaCurve[768];
			const auto& maximum = gammaControl.GammaCurve[1024];
			spdlog::info(
				"Silhouette DXGI gamma: hr=0x{:08X}, scale=({}, {}, {}), offset=({}, {}, {}), "
				"curve256=({}, {}, {}), curve512=({}, {}, {}), curve768=({}, {}, {}), curve1024=({}, {}, {})",
				static_cast<std::uint32_t>(gammaControlResult),
				gammaControl.Scale.Red,
				gammaControl.Scale.Green,
				gammaControl.Scale.Blue,
				gammaControl.Offset.Red,
				gammaControl.Offset.Green,
				gammaControl.Offset.Blue,
				quarter.Red,
				quarter.Green,
				quarter.Blue,
				middle.Red,
			...
			spdlog::info(
				"Silhouette DXGI gamma unavailable: hr=0x{:08X} (normal for many windowed swap chains)",
				static_cast<std::uint32_t>(gammaControlResult));
		}
	}
...
void CDoF::DoFRenderer::ApplySilhouette(RE::RENDER_TARGET a_outputTarget)
{
	try {
		...
		if (!loggedFirstSilhouetteFrame_) {
			...
			if (a_outputTarget == RE::RENDER_TARGETS::kFRAMEBUFFER) {
				LogSilhouetteOutputDiagnostics(

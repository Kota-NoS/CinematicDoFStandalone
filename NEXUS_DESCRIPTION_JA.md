...
バージョン1.0.2は、「空を鮮明に保つ」に遠端深度の月の保護を追加し、独立した2色のシルエット撮影モードと、0.1～4000mを扱える対数操作の「ピント距離」を収録します。実深度を併用する人物保護、地形・水面との滑らかな境界、Skyrim 1.6.1170／1.7.104対応、従来のプリセット／INI互換性も維持します。
...
- 1つの「空を鮮明に保つ」で、深度が未描画の空と遠端深度の月をDoFから除外
- DoF本体がOFFでも使える、独立した2色のシルエット撮影モード
- 近距離の精度を保ったまま0.1～4000mを扱える対数操作の「ピント距離」
...
- 空と月の除外：「詳細設定を表示」の右側にある「空を鮮明に保つ」で切り替えます。月保護も意図的に同じスイッチへ連動します。互換用の基準初期値はOFFです。既存INI／プリセットに項目がない場合もOFFとして読み込みます。
